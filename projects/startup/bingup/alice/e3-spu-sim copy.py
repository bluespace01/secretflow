...
# alice: 
#   ray start --head --node-ip-address="172.17.0.2" --port="9001" --resources='{"alice": 16}' --include-dashboard=False --disable-usage-stats
#   
# bob:
#   ray start --address="172.17.0.2:9001" --resources='{"bob": 16}' --disable-usage-stats
...
# 初始化 SecretFlow
# 请确保 '172.17.0.2:9001' 是 Ray head node 的地址
sf.init(parties=['alice', 'bob'], debug_mode=True, address='172.17.0.2:9001')
...
cluster_def = {
    'nodes': [
        {
            'party': 'alice',
            'address': '172.17.0.2:9110',  # 确保此地址和端口未被占用
            'listen_addr': ''      # 确保此地址和端口未被占用
        },
        {
            'party': 'bob',
            'address': '172.17.0.3:9210',  # 确保此地址和端口未被占用
            'listen_addr': ''      # 确保此地址和端口未被占用
        },
    ],
    'runtime_config': {
